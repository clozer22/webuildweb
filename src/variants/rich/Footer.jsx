...
export default function Footer({ onNavClick }) {
  // Inline SVG icons for official social media platforms
  const LinkedInIcon = () => (
  ...
  return (
    <footer className="bg-[#0A0A0A] border-t border-[#2A2A2A] relative overflow-hidden text-gray-400">
      
      {/* Subtle Glow at top */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-px bg-gradient-to-r from-transparent via-[#D4FF00]/50 to-transparent" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
          ...
          <div className="md:col-span-5 space-y-6">
            ...
            <div className="p-3 bg-[#1E1E1E] rounded-xl border border-[#2A2A2A] inline-flex items-center gap-2 text-xs font-semibold text-white uppercase tracking-wider">
          ...
          <div className="md:col-span-3 space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-white">Sitemap Navigation</h4>
            <ul className="space-y-2.5 text-xs">
              {quickLinks.map((link) => (
                <li key={link.id}>
                  <button
                    onClick={() => onNavClick(link.id)}
                    className="hover:text-[#D4FF00] transition-colors cursor-pointer flex items-center gap-1.5"
                  >
                    <span>&rsaquo;</span>
          ...
          {/* Col 3: Social Media Links & Founders (4 cols) */}
          <div className="md:col-span-4 space-y-6">
              ...
              {socialLinks.map((social) => {
                ...
                return (
                  <a
                    ...
                    className="p-3 rounded-2xl bg-[#1E1E1E] border border-[#2A2A2A] text-gray-300 hover:text-[#0A0A0A] hover:bg-[#D4FF00] hover:border-[#D4FF00] transition-all duration-300 shadow-md hover:scale-110 group relative"
            ...
            <div className="p-4 rounded-2xl bg-[#141414] border border-[#2A2A2A] text-xs text-[#7A7A7A]">
              <div className="font-semibold text-white mb-1">Founders & Leadership</div>
        ...
        <div className="pt-8 border-t border-[#1E1E1E] flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[#7A7A7A]">
          <div>
            &copy; {new Date().getFullYear()} WeBuildWeb (WBW). All rights reserved.
          </div>
          <div className="flex items-center gap-1">
            <span>Built with precision by</span>
            <span className="text-white font-semibold">Mark Diez Aballe</span>
            <span>&amp;</span>
            <span className="text-[#D4FF00] font-semibold">Yzelle Lim</span>
          </div>
        </div>
